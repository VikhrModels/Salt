...
class SaltDataset(ABC, Dataset):
    def __init__(
        self, config: DictConfig, dataset: SizedDataset, tokenizer: PreTrainedTokenizer
    ):
        self.dataset = dataset
        self.tokenizer = tokenizer

        self.start_audio_token_id = torch.tensor([[config.start_audio_token_id]])
        self.end_audio_token_id = torch.tensor([[config.end_audio_token_id]])
        self.bos_id = torch.tensor([[config.bos_id]])
        self.eos_id = torch.tensor([[config.eos_id]])
