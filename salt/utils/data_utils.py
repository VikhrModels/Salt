...
from salt.data.instruct_datasets import (
    SaltTextToSpeechDatasetWithVoiceDescription,
...
def filter_long_audio(dataset: DatasetDict, max_audio_tokens: int) -> DatasetDict:
    train = dataset["train"].filter(
        lambda item: len(item["audio_tokens"]) <= max_audio_tokens
    )
    val = dataset["validation"].filter(
        lambda item: len(item["audio_tokens"]) <= max_audio_tokens
    )
    return DatasetDict({"train": train, "validation": val})


def get_dataset_from_type(
    ...
    if dataset_type != DataType.text:
        dataset = filter_long_audio(dataset, config.max_audio_tokens)

    if dataset_type == DataType.audio:
        train = SaltAudioDataset(config, dataset["train"], tokenizer)
        val = SaltAudioDataset(config, dataset["validation"], tokenizer)
    elif dataset_type == DataType.text:
        train = SaltTextDataset(config, dataset["train"], tokenizer)
        val = SaltTextDataset(config, dataset["validation"], tokenizer)
    elif dataset_type == DataType.tts:
        train = SaltTextToSpeechDataset(config, dataset["train"], tokenizer)
        val = SaltTextToSpeechDataset(config, dataset["validation"], tokenizer)
    elif dataset_type == DataType.asr:
        train = SaltSpeechRecognitionDataset(config, dataset["train"], tokenizer)
        val = SaltSpeechRecognitionDataset(config, dataset["validation"], tokenizer)
    elif dataset_type == DataType.instruct_tts:
        train = SaltTextToSpeechDatasetWithVoiceDescription(
            config, dataset["train"], tokenizer
        )
        val = SaltTextToSpeechDatasetWithVoiceDescription(
            config, dataset["validation"], tokenizer
        )
    else:
        train = SaltSpeechRecognitionDatasetWithVoiceDescription(
            config, dataset["train"], tokenizer
        )
        val = SaltSpeechRecognitionDatasetWithVoiceDescription(
            config, dataset["validation"], tokenizer
        )
...
def prepare_data(config: DictConfig, tokenizer: PreTrainedTokenizer):
    ...
    for data_config in config.datasets:
        data_type = DataType(data_config.type)
        train, val = get_dataset_from_type(
            data_type, data_config.path, config, tokenizer
        )
